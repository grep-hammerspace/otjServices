...
4) ~~Use an opensource/free model instead of relying on Anthropic - OpenRouter Nvidia Nim?~~ DONE — calls an OpenAI-compatible endpoint, defaults to a free OpenRouter model. Configured by `LLM_BASE_URL` / `LLM_MODEL` / `LLM_API_KEY`, so swapping to Groq/NIM/Ollama needs no code change.
