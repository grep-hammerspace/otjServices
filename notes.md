...
3) ~~Upgrade to AWS, with free tier, not sure how that looks with running privileged containers though, will have to do some more digging~~ — resolved, see `deployment-migration-plan.html` (moved tailscaled to the host + `tailscale serve` so hours-api no longer needs elevated container privileges)
